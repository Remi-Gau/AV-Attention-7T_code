function Analysis_FFX_with_baseline()
...
for SubjInd = 2:size(SubjectList,1)
    ...
    AnalysisFolder = fullfile(SubjectFolder, 'FFX_baseline');
    ...
    for RunInd = 1:NbRuns
        ...
        for i = 1:size(Stim_Time{1,2},1)
            ...
            if strcmp(Stim_Time{1,1}(i,:), 'Attend2Audio_Fixation')
                ...
                BlockType = [];
                CurrentCondition = 1;
                BaselineDur{RunInd}(end+1) = Stim_Time{1,2}(i)-Baseline{RunInd}(end);
            ...
            elseif strcmp(Stim_Time{1,1}(i,:), 'Stimulus_Offset')
                if StimCount == 50
                    ...
                    StimCount = 0;
                    Baseline{RunInd}(end+1) = Stim_Time{1,2}(i);
                end
        ...
        BaselineDur{RunInd}(end+1) = NbVol(RunInd)*TR-Baseline{RunInd}(end);
    ...
    for RunInd = 1:NbRuns
        ...
        if numel(IMAGES_ls)==0
            try
                File2Unzip = spm_select('FPList', fullfile(pwd),...
                    ['^' strrep(PrefixImages2Select, '*', '.*') '.gz$']);
                gunzip(File2Unzip)
                                File2Unzip = spm_select('FPList', fullfile(pwd),...
                    ['^' strrep(PrefixImages2Select, '*', '.*') '.gz$']);
                IMAGES_ls = dir(PrefixImages2Select);                
            catch
                error('No file found for this run')
            end
        end
        ...
        matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,RunInd).cond(1,ConditionNumber).name = 'Baseline';
        matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,RunInd).cond(1,ConditionNumber).duration =  BaselineDur{RunInd};
        matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,RunInd).cond(1,ConditionNumber).tmod = 0;
        matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,RunInd).cond(1,ConditionNumber).pmod=struct('name',{},'param',{}, 'poly', {});
        matlabbatch{1,1}.spm.stats.fmri_spec.sess(1,RunInd).cond(1,ConditionNumber).onset = Baseline{RunInd};
...
save ('All_SOTs_baseline.mat', 'All_SOTs', 'All_Durations')
end
...
save (strcat('FFX_baseline_Subject_', SubjID, '_jobs'), 'matlabbatch');
end
