...
%     Logic = any([CdtVec==2 ; CdtVec==5]);
    ...
    [~,I_V1] = sort(nanmean(Cst_SeedROI(Logic,:)));
    IdxToAvg = floor(linspace(1,numel(I_V1),40+1));
    
    % split by activated and deactivated vertives
    Act_deact_Seed = nanmean(Cst_SeedROI(Logic,:))>0;
    ...
    for iCV = 1:2
        ...
        for iCdt = 1:6
            ...
            Train_rows = all([Row2Select train_ses],2);
            ...
            for iVert = 1:size(Cst_SeedROI,2)
                X = Cst_SeedROI(Train_rows,iVert); % take the right condition S paramaters of V1
            ...
            B_act(iSub,iCdt,:,1,iCV) = nanmean(Vert_BOLD(:,1,Act_deact_Seed==1),3);
            B_act(iSub,iCdt,:,2,iCV) = nanmean(Vert_BOLD(:,2,Act_deact_Seed==1),3);
            Seed_Act(iSub,iCV) = nanmean(nanmean(Cst_SeedROI(Train_rows,Act_deact_Seed==1))); %Keep the mean activation across blocks
            
            % On deactivated vertices same as above
            B_deact(iSub,iCdt,:,1,iCV) = nanmean(Vert_BOLD(:,1,Act_deact_Seed==0),3);
            B_deact(iSub,iCdt,:,2,iCV) = nanmean(Vert_BOLD(:,2,Act_deact_Seed==0),3);
            Seed_Deact(iSub,iCV) = nanmean(nanmean(Cst_SeedROI(Train_rows,Act_deact_Seed==0)));
            ...
            for iSplit = 2:numel(IdxToAvg)
                Vert_2_select = I_V1(IdxToAvg(iSplit-1):IdxToAvg(iSplit)); %specifies which vertex to take for that bin
                ...
                Seed_Act_rank(iSub,iSplit,iCdt,iCV) = nanmean(nanmean(Cst_SeedROI(Train_rows,Vert_2_select)));
            end
        ...
        for iCdt = 1:2
            ...
            % On activated vertices
            B_act_att(iSub,iCdt,:,1,iCV) = nanmean(Vert_BOLD(:,1,Act_deact_Seed==1),3);
            B_act_att(iSub,iCdt,:,2,iCV) = nanmean(Vert_BOLD(:,2,Act_deact_Seed==1),3);
            Seed_Act_att(iSub,iCV) = nanmean(nanmean(Cst_SeedROI(Train_rows,Act_deact_Seed==1)));
            
            % On deactivated vertices same as above
            B_deact_att(iSub,iCdt,:,1,iCV) = nanmean(Vert_BOLD(:,1,Act_deact_Seed==0),3);
            B_deact_att(iSub,iCdt,:,2,iCV) = nanmean(Vert_BOLD(:,2,Act_deact_Seed==0),3);
            Seed_Deact_att(iSub,iCV) = nanmean(nanmean(Cst_SeedROI(Train_rows,Act_deact_Seed==0)));
            ...
            for iSplit = 2:numel(IdxToAvg)
                ...
                Seed_Act_rank_att(iSub,iSplit,iCV) = nanmean(nanmean(Cst_SeedROI(Train_rows,Vert_2_select)));
            end
        ...
        for iCdt = 1:2
            ...
            % On activated vertices
            B_act_sens(iSub,iCdt,:,1,iCV) = nanmean(Vert_BOLD(:,1,Act_deact_Seed==1),3);
            B_act_sens(iSub,iCdt,:,2,iCV) = nanmean(Vert_BOLD(:,2,Act_deact_Seed==1),3);
            Seed_Act_sens(iSub,iCV) = nanmean(nanmean(Cst_SeedROI(Train_rows,Act_deact_Seed==1)));
            
            % On deactivated vertices same as above
            B_deact_sens(iSub,iCdt,:,1,iCV) = nanmean(Vert_BOLD(:,1,Act_deact_Seed==0),3);
            B_deact_sens(iSub,iCdt,:,2,iCV) = nanmean(Vert_BOLD(:,2,Act_deact_Seed==0),3);
            Seed_Deact_sens(iSub,iCV) = nanmean(nanmean(Cst_SeedROI(Train_rows,Act_deact_Seed==0)));
            ...
            for iSplit = 2:numel(IdxToAvg)
                ...
                Seed_Act_rank_sens(iSub,iSplit,iCV) = nanmean(nanmean(Cst_SeedROI(Train_rows,Vert_2_select)));
            end
...
save(fullfile(Results_Folder,strcat('Connectivity_A12V1_Surf_', num2str(NbLayers), '_layers.mat')), ...
