...
Mask.ROI(1) = struct('name', 'A1_V_Deact_surf', 'fname', 'A1_V_deact.nii');
...
% Mask.ROI(1) = struct('name', 'A1_V_Act', 'fname', 'A1_surf_V_Stim>Baseline_thres_0.1.nii');
% Mask.ROI(end+1) = struct('name', 'A1_V_Deact', 'fname', 'A1_surf_V_Stim<Baseline_thres_0.1.nii');
% Mask.ROI(end+1) = struct('name', 'PT_V_Act', 'fname', 'PT_surf_thres_V_Stim>Baseline_thres_0.1.nii');
% Mask.ROI(end+1) = struct('name', 'PT-V_Deact', 'fname', 'PT_surf_thres_V_Stim<Baseline_thres_0.1.nii');
...
% Mask.ROI(end+1) = struct('name', 'PT_surf_thres', 'fname', 'PT_surf_thres.nii');
...
%Mask.ROI(end+1) = struct('name', 'pSTG_BT', 'fname', 'rrw_pA22c_BrainTome.nii');
%Mask.ROI(end+1) = struct('name', 'PT_BT', 'fname', 'rrw_pA41-42_BrainTome.nii');
...
%Mask.ROI(end+1) = struct('name', 'S1_AAL', 'fname',  'rrwS1_AAL.nii');
%Mask.ROI(end+1) = struct('name', 'S1_Cyt', 'fname',  'rrwROI_S1_Cyt_MNI.img');

%Mask.ROI(end+1) = struct('name', 'HG_STG', 'fname',  'rrwSTG_AAL.nii');
%Mask.ROI(end+1) = struct('name', 'STG', 'fname',  'rrwHG_STG_AAL.nii');
%Mask.ROI(end+1) = struct('name', 'STG_Post', 'fname',  'rrwSTG_Post_AAL.nii');
...
%Mask.ROI(end+1) = struct('name', 'TE', 'fname', 'rrwTE_MNI.nii');
...
%Mask.ROI(end+1) = struct('name', 'V2-3', 'fname', 'rrwProbRet_V2-3.nii');
%Mask.ROI(end+1) = struct('name', 'V1-2-3', 'fname', 'rrwProbRet_V1-2-3.nii');
%Mask.ROI(end+1) = struct('name', 'V1-2-3d', 'fname', 'rrwProbRet_V1-2-3d.nii');
%Mask.ROI(end+1) = struct('name', 'V1-2-3v', 'fname', 'rrwProbRet_V1-2-3v.nii');
...
%         'Subjects_Data', ['Subject_' SubjID], 'Transfer', 'Profiles', TargetLayerFile);
    ...
    if UseGrpMask
        IndMask = logical(spm_read_vols(Mask.global.hdr));
        GrpMask = fullfile(BackUpFolder, ['Subject_' SubjID '_grp_mask_inv.nii']);
        GrpMask = logical(spm_read_vols(spm_vol(GrpMask)));
        Mask.global.img = all(cat(4,IndMask,GrpMask),4);
        clear IndMask GrpMask
    else
...
%     CstFiles = spm_vol(CstFiles);
    ...
    load(fullfile(StartFolder, 'Figures','Behavioral', 'BehavioralResults.mat'), 'GroupResults')
    load(fullfile(StartFolder,'Figures','Behavioral','ExcludeBlocks.mat'), 'ExcludeBlocks')
