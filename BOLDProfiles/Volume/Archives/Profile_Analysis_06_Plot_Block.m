...
cd(FigureFolder)
load(strcat('Data_Block_NoQuadGLM_', num2str(NbLayers), '_Layers', '.mat'), 'AllSubjects_Data')
...
for iROI=1:length(AllSubjects_Data)
    for iCond=1:size(AllSubjects_Data(iROI).Differential.Blocks.Beta.DATA,2)
...
%                 clear ToPlot
        ...
        Name = [AllSubjects_Data(ROI_Ind).name '-Block-DifferentialWithSubjects'];
        ToPlot.shadedErrorBar.mean = flipud(AllSubjects_Data(ROI_Ind).Differential.MEAN);
        ToPlot.shadedErrorBar.errorbar = flipud(AllSubjects_Data(ROI_Ind).Differential.SEM);
        ...
        ToPlot.BOLD_Thresholds = squeeze(BOLD_Thresholds(iROI,:,:));
