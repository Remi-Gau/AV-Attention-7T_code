                    ...
                    for iROI=1:numel(ROIs)
                        ...
                        for i=1:size(Class_Acc,1)
                            for j=1:size(Class_Acc,2)
                                tmp(:,end+1) = squeeze(Class_Acc(i,j).TotAcc(end,:,:,:));
                            end
                        end
                        
                        MVPA_SubjectsData(SubjInd,iSVM,iROI,1:(NbLayers+1)) = mean(tmp,2); 
                        clear tmp 
                        ...
                        for i=1:size(Class_Acc,1)
                            for j=1:size(Class_Acc,2)
                                for iLayer = 2:NbLayers+1
                                    ...
                                    tmp(iLayer-1,:) = mean(Pred==Label,2)';
                                end
                                Acc(:,:,Idx)=tmp;
                                Idx=Idx+1;
                            end
            ...
            for WithPerm = 0
                ...
                for WithSubj=[0 1]
                    ...
                    for iROI=1:numel(ROIs)
                        ...
                        SaveSufix = [SaveSufix 'Smoothing: FWHM 6 mm Subsampled\n'];
