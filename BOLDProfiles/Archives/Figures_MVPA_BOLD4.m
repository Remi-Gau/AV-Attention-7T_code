...
Include = true(size(SubjectList,1),numel(ROIs));
...
for iROI=1:numel(ROIs)
    ...
    for iSVM = 1:numel(Analysis)
        ...
        DATA.Data = fliplr(squeeze(MVPA_SubjectsData(Include(:,iROI),iSVM,iROI,2:(NbLayers+1))));
        DATA.Betas = squeeze(MVPA_SubjectsBetas(Include(:,iROI),iROI,:,iSVM));
        ...
        DATA.Name = char({strrep(ROIs{iROI},'_', ' ');['MVPA: ' Analysis(iSVM).name]});
        
        PlotProfileAndBetasBasic(DATA)
        clear DATA
        
        print(gcf, fullfile(StartDirectory, 'Figures', ...
            [ROIs{iROI} '_MVPA_' num2str(iSVM) '_' num2str(NbLayers) 'Layers_NoQuad.pdf']), '-dpdf')
