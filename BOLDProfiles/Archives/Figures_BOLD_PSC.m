...
for iCond = 1:2
    for iROI=1:length(AllSubjects_Data)
        ...
        tmp = squeeze(AllSubjects_Data(iROI).MainEffectsPSC.Blocks.Beta.DATA(:,iCond,Include))';
        [~,P] = ttest(tmp);
        All_P(iROI,:,Target) = P;
        BOLD_SubjectsBetas(:,iROI,1:size(tmp,2),Target) = tmp;
        
        for iSubj = 1:length(Include)
            BOLD_SubjectsData(iSubj,Target,iROI,1:NbLayers) = AllSubjects_Data(iROI).MainEffectsPSC.DATA(:,iCond,iSubj);
        end
    end
    Target = Target+1;
end
...
for iCond = 7:8
    for iROI=1:length(AllSubjects_Data)
        Include = find(AllSubjects_Data(iROI).Include);
        tmp = squeeze(AllSubjects_Data(iROI).BiVSUniPSC.Blocks.Beta.DATA(:,iCond,Include))';
        [~,P] = ttest(tmp);
        All_P(iROI,:,Target) = P;
        BOLD_SubjectsBetas(:,iROI,1:size(tmp,2),Target) = tmp;
        
        for iSubj = 1:length(Include)
            BOLD_SubjectsData(iSubj,Target,iROI,1:NbLayers) = AllSubjects_Data(iROI).BiVSUniPSC.DATA(:,iCond,iSubj);
        end
    end
    Target = Target+1;
end
...
for iCond = 2
    for iROI=1:length(AllSubjects_Data)
        Include = find(AllSubjects_Data(iROI).Include);
        tmp = squeeze(AllSubjects_Data(iROI).DifferentialPSC.Blocks.MainEffects.Beta.DATA(:,iCond,Include))';
        [~,P] = ttest(tmp);
        All_P(iROI,:,Target) = P;
        BOLD_SubjectsBetas(:,iROI,1:size(tmp,2),Target) = tmp;
        
        for iSubj = 1:length(Include)
            BOLD_SubjectsData(iSubj,Target,iROI,1:NbLayers) = AllSubjects_Data(iROI).DifferentialPSC.MainEffect.DATA(1:NbLayers,iCond,iSubj);
        end
    end
end
...
for iROI=1:numel(ROIs)
    ...
    for iCond = 1:size(BOLD_SubjectsData,2)
        ...
        print(gcf, fullfile(StartDirectory, 'Figures', ...
            [ROIs{iROI} '_' num2str(iCond) '_' num2str(NbLayers) 'Layers_PSC_NoQuad.pdf']), '-dpdf')
