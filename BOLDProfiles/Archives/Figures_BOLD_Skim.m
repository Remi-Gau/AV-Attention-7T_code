...
for iCond = 1:2
    for iROI=1:length(AllSubjects_Data)
        ...
        tmp = squeeze(AllSubjects_Data(iROI).MainEffectsSkim.Blocks.Beta.DATA(:,iCond,Include))';
        [~,P] = ttest(tmp);
        All_P(iROI,:,Target) = P;
        BOLD_SubjectsBetas(:,iROI,1:size(tmp,2),Target) = tmp;
        
        for iSubj = 1:length(Include)
            BOLD_SubjectsData(iSubj,Target,iROI,1:NbLayers) = AllSubjects_Data(iROI).MainEffectsSkim.DATA(:,iCond,iSubj);
        end
    end
    Target = Target+1;
end
...
for iCond = 7:8
    for iROI=1:length(AllSubjects_Data)
        Include = find(AllSubjects_Data(iROI).Include);
        tmp = squeeze(AllSubjects_Data(iROI).BiVSUniSkim.Blocks.Beta.DATA(:,iCond,Include))';
        [~,P] = ttest(tmp);
        All_P(iROI,:,Target) = P;
        BOLD_SubjectsBetas(:,iROI,1:size(tmp,2),Target) = tmp;
        
        for iSubj = 1:length(Include)
            BOLD_SubjectsData(iSubj,Target,iROI,1:NbLayers) = AllSubjects_Data(iROI).BiVSUniSkim.DATA(:,iCond,iSubj);
        end
    end
    Target = Target+1;
end
...
for iCond = 2
    for iROI=1:length(AllSubjects_Data)
        Include = find(AllSubjects_Data(iROI).Include);
        tmp = squeeze(AllSubjects_Data(iROI).DifferentialSkim.Blocks.MainEffects.Beta.DATA(:,iCond,Include))';
        [~,P] = ttest(tmp);
        All_P(iROI,:,Target) = P;
        BOLD_SubjectsBetas(:,iROI,1:size(tmp,2),Target) = tmp;
        
        for iSubj = 1:length(Include)
            BOLD_SubjectsData(iSubj,Target,iROI,1:NbLayers) = AllSubjects_Data(iROI).DifferentialSkim.MainEffect.DATA(1:NbLayers,iCond,iSubj);
        end
    end
end
...
for iROI=1:numel(ROIs)
    ...
    for iCond = 1:size(BOLD_SubjectsData,2)
        ...
        switch iCond
            case 1
                DATA.Name = char({['Skim - ' strrep(ROIs{iROI},'_', ' ')];'A against baseline irrespective of att';'(A_A + A_V)/2'});
            case 2
                DATA.Name = char({['Skim - ' strrep(ROIs{iROI},'_', ' ')];'V against baseline irrespective of att';'(V_A + V_V)/2'});
            case 3
                DATA.Name = char({['Skim - ' strrep(ROIs{iROI},'_', ' ')];'AV-A irrespective of att';'( (AV-A)_A + (AV-A)_V )/2'});
            case 4
                DATA.Name = char({['Skim - ' strrep(ROIs{iROI},'_', ' ')];'AV-V irrespective of att';'( (AV-V)_A + (AV-V)_V )/2'});
            case 5
                DATA.Name = char({['Skim - ' strrep(ROIs{iROI},'_', ' ')];'A att vs V att (irrespective of stim)';'( (A_A - A_V) + (V_A - V_V) + (AV_A - AV_V) )/3'});
        end
        ...
        print(gcf, fullfile(StartDirectory, 'Figures', ...
            [ROIs{iROI} '_' num2str(iCond) '_' num2str(NbLayers) 'Layers_Skim_NoQuad.pdf']), '-dpdf')
