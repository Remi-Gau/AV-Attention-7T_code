function [args, grid] = grid_search_layers(trdata, trlabel, sessions, opt)
...
for i = 1:length(grid.first.acc)
    ...
    elseif strcmp(opt.svm.machine, 'C-SVC')
        args = sprintf('%s -c %g', opt.svm.dargs, grid.first.param(i));  
    end
    if opt.svm.kernel
...
elseif strcmp(opt.svm.machine, 'C-SVC')
    args = sprintf('%s -c %g', opt.svm.dargs, grid.bestparam);
end