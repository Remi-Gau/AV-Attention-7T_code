function PlotInsetFinalAccuracy(DATA)
...
for i=1:size(Betas,2)
    ...
    distributionPlot({Betas(:,i)+.5}, 'xValues', 1.3, 'color', [0.8 0.8 0.8], ...
        'distWidth', 0.4, 'showMM', 0, ...
        'globalNorm', 2)
    
    h = plotSpread(Betas(:,i)+.5, 'distributionIdx', ones(size(Betas(:,i))), ...
...
%     plot([0 1.55], [0 0], ':k', 'LineWidth', .5)
    ...
    h = errorbar(1,nanmean(Betas(:,i)+.5,1),nansem(Betas(:,i)+.5,1), 'ok','LineWidth', 1, 'MarkerSize', 1.5);
