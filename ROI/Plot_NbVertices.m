...
for iSub = 1:NbSub
    ...
    load(fullfile(StartFolder, 'Subjects_Data',['Subj_' SubjectList(iSub,:)  '_ROI_VertOfInt.mat']), 'ROI', 'NbVertex')
    ...
    for hs = 1:2
        ...
        FeatureSaveFile = fullfile(StartFolder, 'Subjects_Data' ,['Subj_' SubjectList(iSub,:)  '_features_' HsSufix 'hs_' ...
            num2str(NbLayers+2) '_surf.mat']);
    ...
    Features_lh(VertexWithDataHS{1},:,:) = MappingBothHS{1}(:,2:end-1,:);
    
    Features_rh = nan(NbVertex(2),NbLayers,size(MappingBothHS{2},3));
    Features_rh(VertexWithDataHS{2},:,:) = MappingBothHS{2}(:,2:end-1,:);
...
ResultsDir = fullfile(StartFolder, 'Results', 'Profiles', 'Surfaces');
...
SavedTxt = fullfile(FigureFolder,'Number_vertices_coverage.csv');
